...
public final class D5_2Perlin implements LFunctionDaDaDa_Da
{
	...
	public final double get(double x, double y, double z) {
		...
		return lerp(w, lerp(v, lerp(u, grad(p[AAA], x, y, z), // AND ADD
		        grad(p[ABA], x - 1, y, z)),                   // BLENDED
		        lerp(u, grad(p[AAB], x, y - 1, z),            // RESULTS
		                grad(p[ABB], x - 1, y - 1, z))),      // FROM 8
		        lerp(v, lerp(u, grad(p[BAA], x, y, z - 1),    // CORNERS
		                grad(p[BBA], x - 1, y, z - 1)),       // OF CUBE
		                lerp(u, grad(p[BAB], x, y - 1, z - 1), grad(p[BBB],
		                        x - 1, y - 1, z - 1))));
	}
	...
	static final int p[] = new int[512], permutation[] = { 151, 160, 137, 91,
